...
def generate_table(alerts, page_size, page_num):
    ...
    for i, alert in enumerate(alerts):
        ...
        table_rows.append(
            ...
            dhc.Tr([
            # dhc.Td(dcc.Checklist(
            #            id={"item": i, "action": "done"},
            #            options=[{"label": "", "value": "create_target"}],
            #            value=False,
            #            style={"display": "inline"},
            # )),
            dhc.Td(dbc.Checkbox(id=f'checkbox-select-alert-{alert_id}')),
...
def update_table(event_trig_num, create_targets, last_page_timestamp, next_page_timestamp, data):
    print('update_table')
    if not data:
        data = {'page_num': 1}
